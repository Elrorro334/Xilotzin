/**
 * QRCode Generator en JavaScript Puro (100% Offline)
 * Adaptado para Cine Xilotzin PWA
 */
(function (global) {
    ...
    for (var j = 8; j < 256; j++) QRMath.EXP_TABLE[j] = QRMath.EXP_TABLE[j - 4] ^ QRMath.EXP_TABLE[j - 5] ^ QRMath.EXP_TABLE[j - 6] ^ QRMath.EXP_TABLE[j - 8];
    ...
    var XilotzinQR = {
        generateSVG: function (text, size) {
            ...
            try {
                ...
                for (var row = 0; row < count; row++) {
                    for (var col = 0; col < count; col++) {
                        if (qr.isDark(row, col)) {
                            svg += '<rect x="' + (col * cellSize).toFixed(2) + '" y="' + (row * cellSize).toFixed(2) + '" width="' + (cellSize + 0.1).toFixed(2) + '" height="' + (cellSize + 0.1).toFixed(2) + '" fill="#000000"/>';
                        }
                    }
                }
                svg += '</svg>';
                ...
                console.warn('QR Local fallback a servicio online:', e);
                return 'https://api.qrserver.com/v1/create-qr-code/?size=' + size + 'x' + size + '&data=' + encodeURIComponent(text);
            }
        }
    };

    global.XilotzinQR = XilotzinQR;
})(window);
